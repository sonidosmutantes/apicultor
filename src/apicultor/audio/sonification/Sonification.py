...
def write_file(filename: str, fs: int, data: np.ndarray) -> bool:
    """Write audio data to file in OGG format via WAV intermediate.
    ...
    try:
        # Write WAV file
        write(wav_path, fs, data)
        
        # Convert WAV to OGG using ffmpeg-python
        (
            ffmpeg
            .input(wav_path)
            .output(ogg_path, acodec='libvorbis', audio_bitrate='192k')
            .overwrite_output()
        ...
        logger.info(f"Successfully wrote {ogg_path}")
        return True
        
    except ffmpeg.Error as e:
        logger.error(f"FFmpeg error writing {filename}: {e.stderr.decode()}")
