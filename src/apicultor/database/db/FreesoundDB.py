...
class FreesoundDB(MirDbApi):
    ...
    def __init__(self):
        pass  # No longer need av_conv reference
    ...
    def get_one_by_mir(self, mir_state):
        try:
            ...
            try:
                wav_output = os.path.splitext(sound.name)[0] + ".wav"
                (
                    ffmpeg
                    .input(sound.name)
                    .output(wav_output, acodec='pcm_s16le', ar=44100)
                    .overwrite_output()
                    .run(capture_stdout=True, capture_stderr=True)
                )
            except ffmpeg.Error as e:
                print(f"FFmpeg error converting {sound.name}: {e.stderr.decode()}")
