...
def find_clicks(song):
    ...
    for frame in song.FrameGenerator():
            ...
            if detections:
                ...
                for start in starts:
                    cutOff = (start + idx_ * song.H) / 44100.
                    frame = song.IIR(frame,cutOff,'low')
                    y.append((start + idx_ * song.H) / 44100.)

                # for end in ends:
                #     y.append((end + idx_ * song.H) / 44100.)
                output.append(frame)
            else:
                output.append(frame)           
    ...
    try:
        ...
        for subdir, dirs, files in os.walk(DATA_PATH):                  
            for f in files:                                           
                ...
                #del without_hum                                         
                db_mag = 20 * np.log10(abs(audio)) #calculate silence if present in audio signal                         
