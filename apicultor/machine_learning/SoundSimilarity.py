...
def plot_similarity_clusters(desc1, desc2, files, plot=None):
    ...
    min_max = preprocessing.scale(
        np.vstack((desc1, desc2)).T, with_mean=False, with_std=False)
    pca = PCA(n_components=2, whiten=True)
    y = pca.fit(min_max).transform(min_max)

    euclidean = AffinityPropagation(
        convergence_iter=1800, affinity='euclidean')
    ...
    if plot == True:
        ...
        plt.scatter(y[euclidean_labels == 0, 0],
                    y[euclidean_labels == 0, 1], c='b')
        plt.scatter(y[euclidean_labels == 1, 0],
                    y[euclidean_labels == 1, 1], c='r')
        plt.scatter(y[euclidean_labels == 2, 0],
                    y[euclidean_labels == 2, 1], c='y')
        plt.scatter(y[euclidean_labels == 3, 0],
                    y[euclidean_labels == 3, 1], c='g')
        plt.show()
    else:
        pass
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/tempo', topdown=False))[-1][-1]:
                if str(t[0]).split('.json')[0] == s.split('tempo.ogg')[0]:
                    shutil.copy(files_dir+'/tempo/'+s,
                                files_dir+'/tempo/'+str(c)+'/'+s)
                    print(str().join((str(t))))
        try:
            simil_audio = [read(files_dir+'/tempo/'+str(c)+'/'+f)[0]
                           for f in list(os.walk(files_dir+'/tempo/'+str(c), topdown=False))[-1][-1]]
            ...
            del simil_x, h
            p = librosa.istft(p)
            write_file(files_dir+'/tempo/'+str(c)+'/remix/' +
                       'similarity_mix_bpm', 44100, p)
            del p
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/mfcc', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('mfcc.ogg')[0]:
                    shutil.copy(files_dir+'/mfcc/'+s,
                                files_dir+'/mfcc/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/mfcc/'+str(c)+'/'+f)[0]
                           for f in list(os.walk(files_dir+'/mfcc/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/mfcc/'+str(c)+'/remix/' +
                       'similarity_mix_mfcc', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/valleys', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('valleys.ogg')[0]:
                    shutil.copy(files_dir+'/valleys/'+s,
                                files_dir+'/valleys/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/valleys/'+str(c)+'/'+f)[0]
                           for f in list(os.walk(files_dir+'/valleys/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/valleys/'+str(c)+'/remix/' +
                       'similarity_mix_valleys', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/centroid', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('centroid.ogg')[0]:
                    shutil.copy(files_dir+'/centroid/'+s,
                                files_dir+'/centroid/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/centroid/'+str(c)+'/'+f)[0] for f in list(
                os.walk(files_dir+'/centroid/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/centroid/'+str(c)+'/remix/' +
                       'similarity_mix_centroid', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/loudness', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('loudness.ogg')[0]:
                    shutil.copy(files_dir+'/loudness/'+s,
                                files_dir+'/loudness/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/loudness/'+str(c)+'/'+f)[0] for f in list(
                os.walk(files_dir+'/loudness/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/loudness/'+str(c)+'/remix/' +
                       'similarity_mix_loudness', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/hfc', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('hfc.ogg')[0]:
                    shutil.copy(files_dir+'/hfc/'+s,
                                files_dir+'/hfc/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/hfc/'+str(c)+'/'+f)[0]
                           for f in list(os.walk(files_dir+'/hfc/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, h
            p = librosa.istft(p)
            write_file(files_dir+'/hfc/'+str(c)+'/remix/' +
                       'similarity_mix_hfc', 44100, p)
            del p
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/inharmonicity', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('inharmonicity.ogg')[0]:
                    shutil.copy(files_dir+'/inharmonicity/'+s,
                                files_dir+'/inharmonicity/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/inharmonicity/'+str(c)+'/'+f)[0] for f in list(
                os.walk(files_dir+'/inharmonicity/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/inharmonicity/'+str(c) +
                       '/remix/'+'similarity_mix_inharmonicity', 44100, h)
            del h
        except Exception as e:
            logger.exception(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/dissonance', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('dissonance.ogg')[0]:
                    shutil.copy(files_dir+'/dissonance/'+s,
                                files_dir+'/dissonance/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/dissonance/'+str(c)+'/'+f)[0] for f in list(
                os.walk(files_dir+'/dissonance/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/dissonance/'+str(c)+'/remix/' +
                       'similarity_mix_dissonance', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/attack', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('attack.ogg')[0]:
                    shutil.copy(files_dir+'/attack/'+s,
                                files_dir+'/attack/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/attack/'+str(c)+'/'+f)[0]
                           for f in list(os.walk(files_dir+'/attack/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, h
            p = librosa.istft(p)
            write_file(files_dir+'/attack/'+str(c)+'/remix/' +
                       'similarity_mix_attack', 44100, p)
            del p
        except Exception as e:
            print(e)
            continue
        ...
        for t in groups[c]:
            for s in list(os.walk(files_dir+'/duration', topdown=False))[-1][-1]:
                if str(t[0]).split('.')[0] == s.split('duration.ogg')[0]:
                    shutil.copy(files_dir+'/duration/'+s,
                                files_dir+'/duration/'+str(c)+'/'+s)
                    print(t)

        try:
            simil_audio = [read(files_dir+'/duration/'+str(c)+'/'+f)[0] for f in list(
                os.walk(files_dir+'/duration/'+str(c), topdown=False))[-1][-1]]
            audio0 = scratch_music(choice(simil_audio))
            audio1 = scratch_music(choice(simil_audio))
            del simil_audio
            audio_N = min([len(i) for i in [audio0, audio1]])
            ...
            del simil_x, p
            h = librosa.istft(h)
            write_file(files_dir+'/duration/'+str(c)+'/remix/' +
                       'similarity_mix_duration', 44100, h)
            del h
        except Exception as e:
            print(e)
            continue
...
def main():
    ...
    try:
        ...
        if not os.path.exists(files_dir+'/descriptores'):
            raise IOError(
                "Must run MIR analysis or set the sounds/tag directory as files_dir to find descriptors directory")
        ...
        desc1, desc2, in1, in2 = get_desc_pair(
            descriptors, files_features, keys)
        euclidean_labels = plot_similarity_clusters(
            desc1, desc2, files, plot=True)
