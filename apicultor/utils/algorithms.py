...
def true_peak_detector(signal):
    def correct_peaks(array):
        polef= 20000
        zerof=14.1e-3
        rpole =1-4*polef / 48000
        rzero = 1-4 * zerof / 48000
        b = np.array([0,-rzero])
        a = np.array([1,rpole])
...
def music_structure_analysis(signal):
    audio = mono_stereo(read(signal)[0])
    song = MIR(audio,48000)
    ...
    ticks = song.ticks * 48000 #bars from BPM
    pcps = []
    ...
    for bar in range(len(ticks)-1):
         ...
         if any(nodes):
            ...
            #print(not any(conditioned_intensity))
            if not any(conditioned_intensity) or not any(mfcc_nodes):
               conditioned_intensity = (onsets_indexes[(ticks[bar] < onsets_indexes)] > ticks[bar+1])
    ...
    while sum([len(i) for i in Cl]) + 1 <= len(W):
        ...
        r += 1e-32
        W += (r * l)
        Cl.append([np.max(Ticks[max_subset])])
    ...
    for i in range(len(Cl)):
        if i == 0:
            segment = audio[:Cl[i]] 
            write_file('audio'+str(i), 48000,segment)
        elif not i == len(Cl) -1:
            segment = audio[Cl[i-1]:Cl[i]] 
            write_file('audio'+str(i), 48000,segment)
        elif i == len(Cl) - 1:
            segment = audio[Cl[i]:] 
            write_file('audio'+str(i), 48000,segment)
