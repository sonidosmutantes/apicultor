            ...
            for class2 in range(class1 + 1, self.n_class):
                ...
                if not type(dccp_cons) == float:                   
                    ...
                    if not type(dccp_cons) == float: 
                        ...
                        old_loss = np.mean((y1-ssd(alpha1, sv1))**2)
                        unprot1 = unprotection_score(old_loss,ssd(alpha1, sv1)),y1)
                        old_loss = np.mean((y2-ssd(alpha2, sv2))**2)
                        unprot2 = unprotection_score(old_loss,ssd(alpha2, sv2)),y2)
                        print("Unprotection score for first class: "+str(unprot1))
                        print("Unprotection score for second class: "+str(unprot2))
                    else: 
...
class svm_layers(deep_support_vector_machines):
    ...
    def layer_computation(self, features, labels, kernel_configs):
        ...
        for i in range(len(self.kernel_configs)):
            ...
            instances = list(combinations(list(range(np.unique(labels))), 2))
